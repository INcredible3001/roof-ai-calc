...
def analyze_image():
    try:
        ...
        # --- НОВАЯ СТРОГАЯ ИНСТРУКЦИЯ (ОТКЛЮЧАЕМ САМОДЕЯТЕЛЬНОСТЬ ИИ) ---
        prompt = """
        Ты профессиональный ИИ-сметчик. Твоя задача — извлечь базовые габариты здания из чертежа для ввода в калькулятор.
        На чертеже изображена 2D проекция крыши (вид сверху).
        
        КРИТИЧЕСКИЕ ПРАВИЛА (Иначе калькулятор сломается):
        1. Длина дома (в метрах) – это размер здания вдоль конька. Если на плане написано 9600, это 9.6 м. НИЧЕГО НЕ ПРИБАВЛЯЙ И НЕ УМНОЖАЙ НА 2. Нам нужна длина одного фасада.
        2. Ширина дома (в метрах) – это размер здания поперек конька (сумма проекций скатов). Если нарисованы два ската по 6850 мм (6.85 м), то Ширина = 6.85 + 6.85 = 13.7 м. 
        ЗАПРЕЩЕНО использовать тригонометрию (синусы/косинусы) для вычисления реальной длины ската. Бери строго плоские цифры проекции с картинки!
        3. Если указано несколько зданий (дом и терраса), бери габариты только главного, самого большого здания.
        4. Угол наклона: если он не указан на чертеже, пиши 30.
        
        ОТВЕТЬ СТРОГО ПО ШАБЛОНУ НИЖЕ (ТОЛЬКО ЦИФРЫ):
        Тип: [Двухскатная / Вальмовая / Односкатная / Шатровая]
        Длина: [число]
        Ширина: [число]
        Угол: [число]
        Ендовы: [число или 0]
        Примыкания: [число или 0]
        """
