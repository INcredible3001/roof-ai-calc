...
def generate_scheme():
    ...
    # Размеры в плане (база)
    w_in = float(data.get("width", 0))   
    h_in = float(data.get("height", 0))  
    
    # Значения, которые уже посчитал JavaScript (в скате)
    ...
    pad = 100 # Увеличили отступ для текста
    ...
    svg += '''<style>
        ...
        .txt {font-family: sans-serif; font-size: 12px; font-weight: bold; fill: #2c3e50;}
        .txt-r {font-family: sans-serif; font-size: 12px; font-weight: bold; fill: #c0392b;}
    </style>'''
    ...
    if roof_type == 'shed':
        ...
        # Подписи
        svg += f'<text x="{cx+w/2}" y="{cy-10}" text-anchor="middle" class="txt">Карниз трап.: {w_in}м | Скат: {s_trap}м</text>'
        svg += f'<text x="{cx-10}" y="{cy+h/2}" text-anchor="middle" transform="rotate(-90,{cx-10},{cy+h/2})" class="txt">Карниз вальмы: {h_in}м | Скат: {s_hip}м</text>'
        svg += f'<text x="{cx+w/2}" y="{cy+h/2-7}" text-anchor="middle" class="txt-r">Конёк: {r_len}м</text>'
        ...
        D = min(w, h) * 0.5 
        D_in = min(w_in, h_in) * 0.5
        # Отрисовка контура (карнизы и торцы)
        svg += f'<path d="M {cx+w/2-D/2} {cy} L {cx+w/2+D/2} {cy} L {cx+w/2+D/2} {cy+h/2-D/2} L {cx+w} {cy+h/2-D/2} L {cx+w} {cy+h/2+D/2} L {cx+w/2+D/2} {cy+h/2+D/2} L {cx+w/2+D/2} {cy+h} L {cx+w/2-D/2} {cy+h} L {cx+w/2-D/2} {cy+h/2+D/2} L {cx} {cy+h/2+D/2} L {cx} {cy+h/2-D/2} L {cx+w/2-D/2} {cy+h/2-D/2} Z" fill="none" stroke="#2ecc71" stroke-width="4"/>'
        # Коньки
        ...
        svg += f'<line x1="{cx+w/2+D/2}" y1="{cy+h/2+D/2}" x2="{cx+w/2}" y2="{cy+h/2}" class="valley"/>'
        svg += f'<line x1="{cx+w/2-D/2}" y1="{cy+h/2+D/2}" x2="{cx+w/2}" y2="{cy+h/2}" class="valley"/>'
        # Стрелки воды в каждом крыле
        svg += f'<line x1="{cx+w/2}" y1="{cy+15}" x2="{cx+w/2-D/4}" y2="{cy+h/2-D/2-5}" class="water" transform="rotate(0)"/>'
        # Подписи
        svg += f'<text x="{cx+w/2}" y="{cy-10}" text-anchor="middle" class="txt">Фронтон: {D_in}м | Скат: {s_trap}м</text>'
        svg += f'<text x="{cx+w/2+D/2+5}" y="{cy+h/2-D/2-5}" class="txt">Ендова</text>'
